...
class Asignacion_plan{
    ...
    public function getAsignacionPlan($periodo){
        $f = new Factory();
        $obj_cp = $f->returnsQuery();
        $query = "select sc.fecha_solicitud, sc.periodo, sc.estado ESTADO_SOLICITUD, em.apellido, em.nombre, cu.nombre NOMBRE_CURSO, pc.fecha_desde, pc.modalidad, es.nombre as ESTADO, es.nro_orden, ap.aprobada, ap.id_asignacion
    ...
    public function getAsignacionPlanBySolicitud($id){
        $f=new Factory();
        $obj_sp=$f->returnsQuery();
        $query="select pc.objetivo, ap.estado, ap.comentarios, ap.viaticos, ap.id_plan, ap.id_asignacion, pc.fecha_desde, pc.modalidad, pc.duracion, pc.unidad, pc.moneda, pc.importe, ap.aprobada
