...
class Cap_Solic
{
    ...
    public static function getCapSolic($periodo){
        ...
        $query="select sc.*, em.apellido EMPLEADO_APELLIDO, em.nombre EMPLEADO_NOMBRE, emx.apellido SOLICITO_APELLIDO, emx.nombre SOLICITO_NOMBRE, em.lugar_trabajo
...
class Asignacion_plan{
    ...
    public function getAsignacionPlan($periodo){
        $f = new Factory();
        $obj_cp = $f->returnsQuery();
        $query = "select sc.fecha_solicitud, sc.periodo, sc.estado ESTADO_SOLICITUD, em.apellido, em.nombre, pc.objetivo, pc.fecha_desde, pc.modalidad, es.nombre as ESTADO, es.nro_orden, ap.aprobada, ap.id_asignacion
                  from asignacion_plan ap, solicitud_capacitacion sc, empleados em, plan_capacitacion pc, cursos cu, estados es
    ...
    public function getAsignacionPlanBySolicitud($id){
        $f=new Factory();
        $obj_sp=$f->returnsQuery();
        $query="select pc.objetivo, es.nro_orden, es.nombre as estado, ap.comentarios, ap.viaticos, ap.id_plan, ap.id_asignacion, pc.fecha_desde, pc.modalidad,
                pc.duracion, pc.unidad, pc.moneda, pc.importe, ap.aprobada
                from asignacion_plan ap, plan_capacitacion pc, estados es
